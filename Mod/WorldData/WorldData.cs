...
{
    public const int BlockX = 1000;

    private static Tile[][,] newTileBlocksLeft = { new Tile[BlockX, Main.maxTilesY] };
    private static Tile[][,] newTileBlocksRight = { new Tile[BlockX, Main.maxTilesY] };
...
public class WorldData
{
    ...
    /// <summary>
    /// 获取当前正在生成的块靠近世界原点的边界的坐标
    /// </summary>
    /// <returns></returns>
    public static int GetGeneratingXOfBlockStart()
    {
        if (nowGenerating == 0) return 0;
        if (nowGenerating > 0)
        {
            return nowGenerating * ExtendingMap.BlockX;
        }
        return (nowGenerating + 1) * ExtendingMap.BlockX;
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static int GetXOfBlockStart()
    {
        if (nowBlock == 0) return 0;
        if (nowBlock > 0)
        {
            return nowBlock * ExtendingMap.BlockX;
        }
        return (nowBlock + 1) * ExtendingMap.BlockX;
    }
    ...
    /// <param name="x"></param>
    /// <returns></returns>
    public static int ConvertGlobalXToBlockX(int x)
    {
        if (nowBlock == 0) return x;
        if (nowBlock > 0)
        {
            return x - nowBlock * ExtendingMap.BlockX;
        }
        return x - (nowBlock + 1) * ExtendingMap.BlockX;
    }
}
